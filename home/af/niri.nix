...
let
  dmsEnabled =
    ...
    || (lib.attrByPath [ "programs" "dank-material-shell" "enable" ] false osConfig);
  noctaliaEnabled =
    (lib.attrByPath [ "programs" "noctalia-shell" "enable" ] false config)
    || (lib.attrByPath [ "programs" "noctalia-shell" "enable" ] false osConfig);
  ...
  replacementPairs = lib.zipLists shellBindLinesBase shellBindLines;
  patchedConfig = lib.foldl'
    (acc: pair: builtins.replaceStrings [ pair.fst ] [ pair.snd ] acc)
    baseConfig
    replacementPairs;
